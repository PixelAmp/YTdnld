...
This was taken from the main pytube and edited to give ability to download audio
"""
import json
import logging
import re
...
class Playlist(object):
    ...
    def download_all(
        ...
        reverse_numbering=False,audio_only=False,
    ):
        """Download all the videos in the the playlist. Initially, download
        resolution is 720p (or highest available), 
        ...
        for link in self.video_urls:
            ...
            dl_stream = yt.streams.filter(
                only_audio=audio_only,subtype='mp4',progressive=(not audio_only)
            ).order_by('abr').desc().first()

            logger.debug('now downloading %s'%yt.title)
            print('now downloading %s'%yt.title)
            if prefix_number:
